...
- IPC handler naming follows 'feature:action' pattern (e.g., 'git:createBranch', 'file:saveMarkdown', 'metrics:query') with methods organized by feature namespace in window.puffin (e.g., window.puffin.git.*, window.puffin.claude.*, window.puffin.file.*). Plugin extensions use qualified naming: ${pluginName}:${extensionName}. All IPC methods exposed through window.puffin namespace via preload.js bridge. Renderer checks success flag from IPC responses to handle errors.
