...
- IPC handlers return standardized response format: { success: boolean, data/result/designs?: any, error?: string }. Callers must check success flag and extract the appropriate property (data, result, or designs depending on handler). Never assume response structure is the raw data—always destructure the envelope. Always destructure nested properties like result.designs or result.stories.
...
- New modal types must be registered in componentManagedModals list in modal-manager.js for proper lifecycle handling. Without registration, the modal manager clears modal content and shows 'Loading...' instead of letting the component render its own content.
- SAM acceptors that clear pending flags (e.g. _pendingSprintPlanning) must be called synchronously during state change, never in async callbacks. Async callbacks that immediately dispatch state-clearing actions create re-entry conditions.
- New SAM action types must be added to TWO places: persistActions whitelist array AND handler condition block in state-persistence.js. Missing from either location causes silent persistence failure with no error indication.
- Plugin actions use qualified format 'pluginName:actionName' (e.g., 'designer-plugin:listDesigns'). IPC handlers use format 'plugin:pluginName:channel'. Core must reference plugins by full plugin ID (e.g., 'designer-plugin', not 'designer').
- Plugin file storage locations follow .puffin subdirectory pattern: .puffin/gui-definitions/ (shared with core), .puffin/claude-plugins/, .puffin/plugin-state.json. File system is the contract between plugin and core.

### Architectural Decisions

- Async state handlers (handleSprintPlanning, handleRerunRequest, handleContinueRequest) called from onStateChange must use guard flags to prevent re-entry conditions that cause state change churn and UI freezes. Pattern: if (state._pending && !this._handling) { this._handling = true; handler().finally(() => this._handling = false) }. Without guard flags, concurrent executions create infinite loops causing app freezes.
- Plugin↔Core communication should be file-system based through shared directories (e.g., .puffin/gui-definitions/ for designer). Plugins and core should not share state objects. Core knows nothing about plugin internals; plugins manage their own state and IPC handlers. Plugin system must operate independently from core SAM, with plugins and core interacting solely through file system and IPC, never through shared state. This ensures proper separation of concerns and plugin independence.
- Component views that depend on dynamic data should reload that data when the view comes into focus, rather than loading at app startup. This provides fresh data and handles race conditions where data hasn't been initialized yet. Pattern: track current view and trigger refresh on focus change.
- Frontend components should avoid calling plugin IPC during init() before plugins are activated. Database initialization and plugin activation occur after component initialization. Solution: reload data on view focus when plugin is guaranteed active, or use exponential backoff retry (deprecated approach).
