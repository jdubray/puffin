...
- **Pluggable Agent Backend (`PUFFIN_AGENT_CMD`)**: Set the `PUFFIN_AGENT_CMD` environment variable to replace the Claude Code CLI subprocess with any compatible agent. All seven spawn sites (`submit`, `isAvailable`, `getVersion`, `generateTitle`, `sendPrompt`, `deriveStories`, `generateInspectionAssertions`) now use the configured command. Falls back to `claude` when the variable is not set — no breaking change for existing users. Primary use case: [deepagents](https://github.com/your-org/local-llm), a LangGraph-based framework for running local LLMs via Ollama.
