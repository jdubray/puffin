...
- Clearly state that Puffin is a tracking/orchestration layer, not an execution layer
Git hook awareness 
- Consider warning users about executable hooks in opened repositories

For Future Development
If adding AI features - Follow "Secure for AI" principle with HITL (human-in-the-loop) for any file operations
Context sanitization - When passing tracked content to 3CLI, clearly mark boundaries between system and user content
Electron hardening - Audit Electron security settings (contextIsolation, nodeIntegration, webSecurity)
---
