...
class BoardRuntime {
  ...
  /**
   * @private
   * A Puffin-bundled machine dir — every project gets the default workflow
   * without carrying machine artifacts of its own.
   *
   * @param {'task-card'|'generation'} [machineId]
   */
  _bundledMachineDir(machineId = 'task-card') {
    return path.resolve(__dirname, '..', '..', 'machines', machineId)
  }
  ...
  _resolveConfig() {
    ...
    const machineEntry = (machineId) => {
      const dir = this._bundledMachineDir(machineId)
      return `{
    machineId: '${machineId}',
    module: '${posix(path.join(dir, 'next.cjs'))}',
    contract: '${posix(path.join(dir, 'contract.json'))}',
    invariants: '${posix(path.join(dir, 'invariants.mjs'))}',
  }`
    }
    const generated = `// Generated by Puffin — the default workflow board.
// Cards are instances of task-card; a queued phase is an instance of
// generation, the batch that owns those cards and enforces its escalation
// policy. To customise the workflow, add your own polyrun.config.mjs at the
// project root; Puffin prefers it over this file.
'use strict';

export default {
  store: { sqlite: '${posix(path.join(this.projectPath, '.puffin', 'board.sqlite'))}' },
  machines: [${machineEntry('task-card')}, ${machineEntry('generation')}],
  handlers: {},
  worker: { leaseMs: 4000 },
...
`
  ...
  getStatus() {
    ...
    // Both machines, because the config registers both: a missing generation
    // machine would otherwise surface as a failed queue rather than a status.
    const hasBundledMachine = ['task-card', 'generation']
      .every(id => fs.existsSync(path.join(this._bundledMachineDir(id), 'next.cjs')))
    return {
      running: !!this._proc,
  ...
  /** Create a generation (the batch a queued phase runs as, born drafting). */
  createGeneration(instanceId) {
