...
class ClaudeMdGenerator {
  constructor() {
    ...
    this.claudeDir = null
    /** @type {string|null} The branch currently reflected in CLAUDE.md */
  ...
  async generateAll(state, activeBranch, getSkillContent = null, getAgentContent = null) {
    ...
    // Scaffold bundled slash commands (e.g. /puffin-sync) into the project
    await this.scaffoldCommands()
  }
  ...
  async scaffoldCommands() {
    ...
    const targets = [
      { src: path.join(templatesDir, 'command.md'), dest: path.join(this.claudeDir, 'commands', 'puffin-sync.md') },
      { src: path.join(templatesDir, 'script.js'), dest: path.join(this.claudeDir, 'scripts', 'puffin-sync.cjs') }
    ]

    // Remove a stale .js copy left by earlier versions that emitted CommonJS
    // under a .js extension (breaks in ESM projects).
    try {
      await fs.unlink(path.join(this.claudeDir, 'scripts', 'puffin-sync.js'))
    } catch {
    ...
    for (const { src, dest } of targets) {
      try {
        ...
        console.error(`[ClaudeMd] Failed to scaffold command ${path.basename(dest)} (non-fatal):`, err.message)
      }
    }
  }
  ...
  async generateBase(state) {
    ...
    lines.push('You may append branch-specific notes after the `<!-- puffin:generated-end -->` marker; those additions are preserved across branch switches and regenerations.')
    lines.push('')
  ...
  async generateBranch(branch, state, skillContent = '', agentContent = '') {
    ...
    // saved in the previous version of this branch file.  This ensures
    // Claude Code's /memory contributions survive Puffin regenerations.
    content += `\n${GENERATED_END_SENTINEL}\n`
    const branchFilePath = path.join(this.claudeDir, `CLAUDE_${branch}.md`)
    const savedAdditions = await this._readSavedClaudeAdditions(branchFilePath)
    if (savedAdditions) {
      content += '\n' + savedAdditions
    }

    await fs.writeFile(branchFilePath, content, 'utf-8')
  }
   ...
   *
   * Before overwriting CLAUDE.md, any content that Claude Code appended
   * after the sentinel in the current file is saved back into the departing
   * branch file so it survives the switch.
   *
   * @param {string} branch - Branch to activate
   */
  async activateBranch(branch) {
    // Capture any Claude Code additions from the current CLAUDE.md before
    // overwriting it.  This must run even when re-activating the same branch
    // (e.g. after generateAll regenerates branch files) so that additions
    // already present in CLAUDE.md are saved back into the branch file before
    // the file is read and the new CLAUDE.md is assembled.
    if (this._activeBranch) {
      await this.captureClaudeMemory(this._activeBranch)
    }
    ...
    // Branch file already contains the sentinel (and any Claude Code additions
    // after it), so a plain concatenation is all that is needed.
  ...
  /**
   * Capture any content Claude Code appended after the sentinel in the
   * current CLAUDE.md and save it back into the branch file so it is
   * not lost when Puffin regenerates or switches branches.
   *
   * Safe to call even when there is nothing to capture (no-op).
   *
   * @param {string} branch - The branch whose file should be updated
   */
  async captureClaudeMemory(branch) {
    if (!this.claudeDir || !branch) return

    const activePath = path.join(this.claudeDir, 'CLAUDE.md')
    let activeContent = ''
    try {
      activeContent = await fs.readFile(activePath, 'utf-8')
    } catch {
      return // No active CLAUDE.md — nothing to capture
    }

    const additions = this._extractClaudeAdditions(activeContent)
    if (!additions) return // Nothing was added by Claude Code

    const branchFilePath = path.join(this.claudeDir, `CLAUDE_${branch}.md`)
    let branchContent = ''
    try {
      branchContent = await fs.readFile(branchFilePath, 'utf-8')
    } catch (err) {
      if (err.code !== 'ENOENT') throw err
      // Branch file doesn't exist yet — nothing to update
      return
    }

    // Replace everything after the sentinel in the branch file
    const sentinelIdx = branchContent.indexOf(GENERATED_END_SENTINEL)
    const puffinPart = sentinelIdx !== -1
      ? branchContent.slice(0, sentinelIdx + GENERATED_END_SENTINEL.length)
      : branchContent.trimEnd() + `\n${GENERATED_END_SENTINEL}`

    const updated = puffinPart + '\n\n' + additions + '\n'
    await fs.writeFile(branchFilePath, updated, 'utf-8')
    console.log(`[CLAUDE.md] Saved Claude Code memory for branch: ${branch}`)
  }

  /**
   * Extract content that Claude Code appended after the sentinel in a
   * CLAUDE.md string, trimmed.  Returns null if nothing was added.
   *
  ...
  /**
   * Read any Claude Code additions already stored after the sentinel in an
   * existing branch file.  Returns empty string when the file does not yet
   * exist or has no post-sentinel content.
   *
   * @param {string} branchFilePath - Absolute path to CLAUDE_{branch}.md
   * @returns {Promise<string>}
   * @private
   */
  async _readSavedClaudeAdditions(branchFilePath) {
    let existing = ''
    try {
      existing = await fs.readFile(branchFilePath, 'utf-8')
    } catch {
      return ''
    }
    return this._extractClaudeAdditions(existing) || ''
  }
...
module.exports.GENERATED_END_SENTINEL = GENERATED_END_SENTINEL
