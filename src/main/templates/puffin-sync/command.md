...
```bash
node .claude/scripts/puffin-sync.cjs [branch-name]
```
...
node .claude/scripts/puffin-sync.cjs

# Sync to UI branch
node .claude/scripts/puffin-sync.cjs ui

# Sync to Backend branch
node .claude/scripts/puffin-sync.cjs backend
```
