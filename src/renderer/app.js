...
class PuffinApp {
  ...
  async getWorkflowSummary() {
    ...
    // Build thread history for the journey timeline (single implicit 'main' stream)
    const allPrompts = this.state?.history?.raw?.branches?.main?.prompts || []
    ...
    const branchHistory = threads.length > 0 ? [{ id: 'main', name: 'Main', threads }] : []
  ...
  /**
   * Update the right swimlane metadata panel with thread stats
   */
  updateMetadataPanel(state) {
    ...
    // Single implicit stream: everything lives in 'main'
    const activePromptId = state.history?.activePromptId
    const branch = state.history?.raw?.branches?.main
