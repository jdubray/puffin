...
describe('task-card machine', () => {
  ...
  it('rejects the illegal drag backlog → implementing', () => {
    const step = fresh()
    const { changed } = step('START_IMPLEMENTATION')
  ...
  it('done is a frozen terminal', () => {
    ...
    for (const action of ['MARK_READY', 'START_IMPLEMENTATION', 'ESCALATE', 'RESUME']) {
